...
run('xcrun', ['swiftc', '-O', '-parse-as-library', '-swift-version', '5', '-target', `${process.arch === 'arm64' ? 'arm64' : 'x86_64'}-apple-macos26.0`, '-I', path.join(products, 'Modules'),
  path.join(root, 'desktop/native/Bridge.swift'), path.join(root, 'desktop/native/AssistantKeychain.swift'), path.join(root, 'desktop/native/MeetingActivity.swift'), ...['AudioCapture.swift', 'Keychain.swift', 'CompanionShortcuts.swift'].map(name => path.join(native, 'App', name)), ...objects,
