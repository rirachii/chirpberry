...
export function SettingsPanel({ runtime, onChange, onCreated }: { runtime: RuntimeSnapshot; onChange(value: RuntimeSnapshot): void; onCreated(meeting: Meeting): void }) {
  ...
  return <div className="settings-panel">
    ...
    <p className="small">Read the next seven days from your Mac calendar to prepare a note. Calendar access starts only when you choose the button below.</p>
    <button className="secondary" disabled={busy || !runtime.capabilities.calendar} onClick={() => void perform(async () => setEvents(await window.chirpberry.calendar()))}>Load upcoming events</button>
    {events.map(event => <div className="calendar-event" key={event.id}><span><strong>{event.title}</strong><small>{new Date(event.start).toLocaleString()}</small></span><button className="secondary" disabled={busy} onClick={() => void perform(async () => onCreated(await window.chirpberry.prepareEvent(event)))}>Prepare note</button></div>)}
