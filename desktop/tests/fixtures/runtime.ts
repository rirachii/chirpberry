...
const adapters: RuntimeAdapters = {
  ...
  createStream: options => ({ connect: async () => { setTimeout(() => options.onEvent({ type: 'transcript.partial', text: 'Synthetic live draft' }), 60); },
    sendAudio: () => true, close: () => {}, finish: async () => { options.onEvent({ type: 'transcript.final', text: 'Synthetic final speech.', event_id: 'fixture-final', timestampMs: 100 }); } }),
