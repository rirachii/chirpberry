...
- Explicit recording/disclosure controls; microphone and optional system audio, live source/translation segments, final-only persistence, pause/resume, bounded provider queues, and stop on error, either notebook or companion closure, or exit. Stop/Pause drain captured PCM before finalizing transcription, including the partial browser-worklet frame. A full Stop upgrades an in-flight Pause; closure, cancellation, or failure immediately aborts draining and disables clipboard delivery.
