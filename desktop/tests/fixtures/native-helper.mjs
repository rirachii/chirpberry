...
input.on('close', () => { record({ event: 'stdin-closed' }); if (mode === 'graceful') process.exit(0); });
input.on('line', line => {
  ...
  const result = command === 'shortcuts.configure' ? { fn: false, failures: [] } : { pid: process.pid, version: 1 };
