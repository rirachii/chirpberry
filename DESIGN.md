...
Ask and Transcript share one optional inspector space. At compact widths it sits below the editor; capture controls remain reachable. Ask offers catch-up, questions, and editable response drafts with reviewable source excerpts and a separate Stop answer action. Share opens a modal showing the exact selected snapshot; personal notes and transcript are opt-in, and chat is excluded. Upcoming shows a quiet chronological agenda with separate Open note and Join actions. Neither starts capture. Apple Calendar setup has one connection action, an optional route to the system Calendar app, and Continue without a calendar. Opt-in call suggestions appear as a nonmodal Start notes/Dismiss banner and never begin recording on their own. Browser activity is only a possible call. See [meeting assistant](docs/meeting-assistant.md) and [call suggestions](docs/meeting-detection.md) for processing and connection boundaries.
