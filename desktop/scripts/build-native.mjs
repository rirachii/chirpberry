...
run('xcrun', ['swiftc', '-O', '-parse-as-library', '-swift-version', '5', '-target', `${process.arch === 'arm64' ? 'arm64' : 'x86_64'}-apple-macos26.0`, '-I', path.join(products, 'Modules'),
  path.join(root, 'desktop/native/Bridge.swift'), ...['AudioCapture.swift', 'Keychain.swift', 'CompanionShortcuts.swift'].map(name => path.join(native, 'App', name)), ...objects,
...
<key>NSCalendarsFullAccessUsageDescription</key><string>Choose an upcoming event to prepare a meeting note. Calendar data stays on this device.</string>
</dict></plist>`);
