...
test('accessible notebook and responsive language examples', async ({page}, info) => {
  await page.goto('/');
  ...
  for (const width of [320, 390, 768, 1440]) {
    ...
    if (width === 320 || width === 1440) await page.screenshot({path: info.outputPath(`website-${width}.png`), fullPage: true});
  }
});

test('both Homebrew controls copy the exact command', async ({page}) => {
  // Exercise copy wiring without modifying the workstation clipboard.
  await page.addInitScript(() => {
    let text = '';
    Object.defineProperty(navigator, 'clipboard', {value: {
      writeText: async value => { text = value; }, readText: async () => text,
    }});
  });
  await page.goto('/');
